...
def getReply(utterance, args):
    ...
    try:
        ...
        # 儲存所有匹配的結果
        all_matches = []
        for query_name in args:
            # 使用 str.contains 進行部分匹配
            matched_rows = df[df['Name'].str.contains(query_name, na=False)]
            
            if not matched_rows.empty:
                # 找到對應的信箱
                for _, row in matched_rows.iterrows():
                    all_matches.append(f"{row['Name']}: {row['Email']}")
        
        if all_matches:
            # 組合所有匹配的結果
            emails_str = "\n".join(all_matches)
            replySTR = f"以下是相關的信箱資訊：\n{emails_str}"
        else:
            names_str = "、".join(args)
            replySTR = f"抱歉，找不到 {names_str} 的信箱資訊"
