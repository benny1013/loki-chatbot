...
def getResult(inputSTR, utterance, args, resultDICT, refDICT, pattern="", toolkitDICT={}):
    ...
    if utterance == "[姜定宇][老師]在[社心][中]有哪些研究專長":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
    ...
    if utterance == "[社心][姜定宇][老師]的研究專長是什麼":
        if CHATBOT:
            replySTR = getReply(utterance, args)
            if replySTR:
                resultDICT["response"] = replySTR
...
if __name__ == "__main__":
    ...
    resultDICT = getResult("姜定宇的研究專長", "[姜定宇]的研究專長", [], {}, {})
    pprint(resultDICT)